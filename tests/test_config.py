...
class ConfigTest(unittest.TestCase):
    ...
    @parameterized.expand([
        ...
        (True,
         [io.StringIO('{properties: {run-interval: 2h, candlesticks: 80}}'),
          io.StringIO('{properties: {run-interval: 2h, candlesticks: 90}}')],
         {'properties': {'run-interval': '2h', 'candlesticks': 90}})
    ])
    @patch('bin_strat.resources.config.open')
    @patch('bin_strat.resources.config.exists')
    def test_yaml_loading(self, exists_local, open_input, expected, exists_mock, open_mock):
    ...
    @patch('bin_strat.resources.config.open')
    @patch('bin_strat.resources.config.exists')
    def test_environment_interpolation(self, env, open_input, expected, exists_mock, open_mock):
        for e in env.keys():
            os.environ[e] = env[e]
